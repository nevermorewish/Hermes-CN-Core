...
|---|---|---|---|---|
| **P-061** | `.github/workflows/release-runtime.yml`、`docs/RUNTIME_RELEASES.md`、`docs/RUNTIME_VERSIONING.md` | 通过 SSH 将 Runtime 清单和压缩包发布到 Linux 承载的 `https://huanxing.ai/downloads/Hermes-CN-Core/runtime`，并让每个清单的 `artifactUrl` 指向该下载服务。 | CN 桌面发行与 Runtime 更新统一使用生产 Linux 下载服务器，移除 TOS 和旧地址。 | CN 部署专属，不提交上游 |
