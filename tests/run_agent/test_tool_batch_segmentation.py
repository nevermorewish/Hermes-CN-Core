...
class TestSegmentedDispatchIntegration:
    ...
    def test_steer_remains_pending_for_unified_reminder_injection(self, agent):
        """Segment execution must not consume the reminder-registry steer.

        The conversation loop injects it once after tool execution; the legacy
        per-tool drain hook is intentionally a no-op on dev-fix.
        """
        calls = [
            _tc("web_search", '{"query":"a"}', call_id="s1"),
            _tc("web_search", '{"query":"b"}', call_id="s2"),
